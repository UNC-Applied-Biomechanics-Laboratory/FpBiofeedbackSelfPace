...
bodyWeight = str2double(answer{4});
...
% run TM
[WarmUpData] = RunTMFixedSpeed(Settings);

% analyze warm up force data
TypicalFp = AnalyzeFp(WarmUpData, 'Yes'); 
TypicalFp.NormMean = TypicalFp.Mean / (bodyWeight * 9.81); 
...
% Run self pace mode
SelfPaceTestData = RunTMSelfPace(Settings);
...
%     Settings.Duration = 300; % duration in seconds
    ...
    SpeedTarget(i).Data = RunTMFixedSpeed(Settings);
    
end

%% Analyze Speed Data
for i = 1:NumTrials
    SpeedTarget(i).FpData = AnalyzeFp(SpeedTarget(i).Data, 'Yes');
...
%     Settings.Duration = 70;
    ...
    FpTarget(i).Data = RunTMSelfPace(Settings);
...
%     Settings.Duration = 70;
    ...
    % Run treadmill and save data
    FixedSpdTargetFp(i).Data = RunTMFixedSpeed(Settings);
